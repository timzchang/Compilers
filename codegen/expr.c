...
void expr_codegen(struct expr *e, FILE *output){
	...
	case EXPR_EQ:
		expr_codegen(e->left, output);
		expr_codegen(e->right, output);
		if((e->left->kind == EXPR_STRING && e->right->kind == EXPR_STRING) || (e->left->kind == EXPR_NAME && e->left->symbol->type->kind == TYPE_STRING) || (e->right->kind == EXPR_NAME && e->left->symbol->type->kind == TYPE_STRING)){
